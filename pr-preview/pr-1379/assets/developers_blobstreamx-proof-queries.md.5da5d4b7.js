import{_ as s,o as n,c as a,Q as l}from"./chunks/framework.7e48ea92.js";const o="/pr-preview/pr-1379/img/blobstream/blobstream-square.png",p="/pr-preview/pr-1379/img/blobstream/blobstream-commitment-diagram.png",m=JSON.parse('{"title":"BlobstreamX proofs queries","description":"Learn how to query the inclusion proofs used in BlobstreamX","frontmatter":{"description":"Learn how to query the inclusion proofs used in BlobstreamX","head":[["meta",{"name":"og:title","content":"BlobstreamX proofs queries | Celestia Docs"},{"name":"og:description","content":false}]]},"headers":[],"relativePath":"developers/blobstreamx-proof-queries.md","filePath":"developers/blobstreamx-proof-queries.md","lastUpdated":1706573747000}'),e={name:"developers/blobstreamx-proof-queries.md"},t=l('<h1 id="blobstreamx-proofs-queries" tabindex="-1">BlobstreamX proofs queries <a class="header-anchor" href="#blobstreamx-proofs-queries" aria-label="Permalink to &quot;BlobstreamX proofs queries&quot;">​</a></h1><h2 id="prerequisites" tabindex="-1">Prerequisites <a class="header-anchor" href="#prerequisites" aria-label="Permalink to &quot;Prerequisites&quot;">​</a></h2><ul><li>Access to a Celestia <a href="./../nodes/consensus-node">consensus full node</a> RPC endpoint (or full node). The node doesn&#39;t need to be a validating node in order for the proofs to be queried. A full node is enough.</li></ul><h2 id="overview-of-the-proof-queries" tabindex="-1">Overview of the proof queries <a class="header-anchor" href="#overview-of-the-proof-queries" aria-label="Permalink to &quot;Overview of the proof queries&quot;">​</a></h2><p>To prove the inclusion of PayForBlob (PFB) transactions, blobs or shares, committed to in a Celestia block, we use the Celestia consensus node&#39;s RPC to query for proofs that can be verified in a Rollup settlement contract via BlobstreamX. In fact, when a PFB transaction is included in a block, it gets separated into a PFB transaction (without the blob), and the actual data blob that it carries. These two are split into shares, which are the low level constructs of a Celestia block, and saved to the corresponding Celestia block. Learn more about shares in the <a href="https://celestiaorg.github.io/celestia-app/specs/shares.html" target="_blank" rel="noreferrer">shares specs</a>.</p><p>The two diagrams below summarize how a single share, which can contain a PFD transaction, or a part of the rollup data that was posted using a PFB, is committed to in BlobstreamX.</p><p>The share is highlighted in green. <code>R0</code>, <code>R1</code> etc, represent the respective row and column roots, the blue and pink gradients are erasure encoded data. More details on the square layout can be found <a href="https://github.com/celestiaorg/celestia-app/blob/v1.1.0/specs/src/specs/data_square_layout.md" target="_blank" rel="noreferrer">in the data square layout</a> and <a href="https://github.com/celestiaorg/celestia-app/blob/v1.1.0/specs/src/specs/data_structures.md#erasure-coding" target="_blank" rel="noreferrer">data structures</a> portion of the specs.</p><h3 id="the-celestia-square" tabindex="-1">The Celestia square <a class="header-anchor" href="#the-celestia-square" aria-label="Permalink to &quot;The Celestia square&quot;">​</a></h3><p><img src="'+o+'" alt="Square"></p><h3 id="the-commitment-scheme" tabindex="-1">The commitment scheme <a class="header-anchor" href="#the-commitment-scheme" aria-label="Permalink to &quot;The commitment scheme&quot;">​</a></h3><p><img src="'+p+`" alt="Blobstream Commitment Diagram"></p><p>So to prove inclusion of a share to a Celestia block, we use BlobstreamX as a source of truth, more information on BlobstreamX can be found in <a href="./">TODO add link</a>. In a nutshell, BlobstreamX attests to the data posted to Celestia in the BlobstreamX contract via verifying a zk-proof of the headers of a batch of Celestia blocks. Then, it keeps reference of that batch of blocks using the merkleized commitment of their <code>(dataRoot, height)</code> resulting in a <code>data root tuple root</code>. Check the above diagram which shows:</p><ul><li>0: those are the shares, that when unified, contain the PFB or the rollup data blob.</li><li>1: the row and column roots are the namespace merkle tree roots over the shares. More information on the NMT in the <a href="https://celestiaorg.github.io/celestia-app/specs/data_structures.html?highlight=namespace%20merkle#namespace-merkle-tree" target="_blank" rel="noreferrer">nmt specs</a>. These commit to the rows and columns containing the above shares.</li><li>2: the data roots: which are the binary merkle tree commitment over the row and column roots. This means that if you can prove that a share is part of a row, using a namespace merkle proof. Then prove that this row is committed to by the data root. Then you can be sure that that share was published to the corresponding block.</li><li>3: in order to batch multiple blocks into the same commitment, we create a commitment over the <code>(dataRoot, height)</code> tuple for a batch of blocks, which results in a data root tuple root. It&#39;s this commitment that gets stored in the BlobstreamX smart contract.</li></ul><p>So, if we&#39;re able to prove that a share is part of a row, then that row is committed to by a data root. Then, prove that that data root along with its height is committed to by the data root tuple root, which gets saved to the BlobstreamX contract, we can be sure that that share was committed to in the corresponding Celestia block.</p><p>In this document, we will provide details on how to query the above proofs, and how to adapt them to be sent to a rollup contract for verification.</p><h2 id="hands-on-lab" tabindex="-1">Hands-on lab <a class="header-anchor" href="#hands-on-lab" aria-label="Permalink to &quot;Hands-on lab&quot;">​</a></h2><p>This part will provide the details of proof generation, and the way to make the results of the proofs queries ready to be consumed by the target rollup contract.</p><div class="tip custom-block"><p class="custom-block-title">NOTE</p><p>For the go client snippets, make sure to have the following replaces in your <code>go.mod</code>:</p><div class="language-go vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang">go</span><pre class="shiki github-dark vp-code-dark"><code><span class="line"><span style="color:#E1E4E8;">replace (</span></span>
...
<span class="line"><span style="color:#E1E4E8;">)</span></span></code></pre><pre class="shiki github-light vp-code-light"><code><span class="line"><span style="color:#24292E;">replace (</span></span>
...
<span class="line"><span style="color:#24292E;">}</span></span></code></pre></div><h3 id="full-example-of-proving-that-a-celestia-block-was-committed-to-by-blobstreamx-contract" tabindex="-1">Full example of proving that a Celestia block was committed to by BlobstreamX contract <a class="header-anchor" href="#full-example-of-proving-that-a-celestia-block-was-committed-to-by-blobstreamx-contract" aria-label="Permalink to &quot;Full example of proving that a Celestia block was committed to by BlobstreamX contract&quot;">​</a></h3><div class="language-go vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang">go</span><pre class="shiki github-dark vp-code-dark"><code><span class="line"><span style="color:#F97583;">package</span><span style="color:#E1E4E8;"> </span><span style="color:#B392F0;">main</span></span>
<span class="line"></span>
<span class="line"><span style="color:#F97583;">import</span><span style="color:#E1E4E8;"> (</span></span>
<span class="line"><span style="color:#E1E4E8;">	</span><span style="color:#9ECBFF;">&quot;</span><span style="color:#B392F0;">context</span><span style="color:#9ECBFF;">&quot;</span></span>
<span class="line"><span style="color:#E1E4E8;">	</span><span style="color:#9ECBFF;">&quot;</span><span style="color:#B392F0;">fmt</span><span style="color:#9ECBFF;">&quot;</span></span>
